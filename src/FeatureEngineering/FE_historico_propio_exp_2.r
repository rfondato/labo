...
# kworking_dir <- "~/buckets/b1/datasets/exp2/"
# kdataset_entrada <- "paquete_premium.csv.gz"
# kdataset_salida  <- "paquete_premium_ext_001.csv.gz"

kworking_dir <- "c:\\data_mining\\datasets\\"
kdataset_entrada <- "paquete_premium_202011.csv"
kdataset_salida  <- "paquete_premium_202011_ext.csv"
...
klags <- c(1,2,3,12)
kdelta_lags <- T
kcruzar_top_vars <- 100
kagregar_bajas_por_mes <- T
ktruncar_n_vars <- 1000
...
AgregarMes  <- function( dataset )
{
  ...
  kcampos_fijos <- c(kcampos_fijos, "mes")
...
CanaritosImportancia  <- function( canaritos_ratio=0.2 )
{
  ...
  return (tb_importancia[ Feature %in% col_utiles ])
}
...
CruzarVariablesImportantes <- function(tb_importancia, n=100, cruzar_con_bajas=TRUE)
{
  gc()
  ...
  if (cruzar_con_bajas) {
    # Agrego numero de BAJA+1 + BAJA+2 para cada mes
    dataset[clase_ternaria != "CONTINUA", bajas_del_mes := .N, by= foto_mes]
  ...
  for( i in 1:length(cols)) {
    for(j in i:length(cols)) {
      nombre = paste0(cols[i], '_vs_', cols[j])
      dataset[, nombre := (get(cols[i]) / get(cols[j]))]
      nuevas_cols = c(nuevas_cols, nombre)
    }
  }
...
ProcesarCombinaciones <- function(importancia) {
  
  # No hago nada si la combinatoria está desactivada
  if (kcruzar_top_vars <= 0) {
    return (importancia)
  }

  # Tomo las X variables mas importantes y las combino entre sí (haciendo el ratio)
  cols_cruzadas = CruzarVariablesImportantes(importancia, kcruzar_top_vars, kagregar_bajas_por_mes)
  
  # Rankeo las nuevas cols cruzadas y las agrego a la lista
  setorder( dataset, foto_mes, numero_de_cliente )
  cols_cruzadas = c(cols_cruzadas, Rankeador( cols_cruzadas ))
  setorder( dataset, numero_de_cliente, foto_mes )
  
  #Tendencias, min/max, avgs, etc para nuevas cols cruzadas
  if (length(kventanas) > 0) {
    for (ventana in kventanas) {
  ...
  if (length(klags) > 0) {
    for (lag in klags) {
      Lags( cols_cruzadas, lag, kdelta_lags ) #calculo los lags de orden "lag"
    }
  }
  
  # Reducción de variables no importantes
  return ( CanaritosImportancia() )
}
...
#cargo el dataset
dataset   <- fread( kdataset_entrada )
...
cols_rankeables  <- copy( setdiff( colnames(dataset), kcampos_fijos ) )

# Rankea las variables dentro de cada mes
setorder( dataset, foto_mes, numero_de_cliente )
Rankeador( cols_rankeables )
setorder( dataset, numero_de_cliente, foto_mes )

# Reducción de variables no importantes
importancia = CanaritosImportancia()

if (length(kventanas) > 0) {
  cols_tendencias  <- copy( setdiff( colnames(dataset), kcampos_fijos ) )
  
  for (ventana in kventanas) {
    # Agrego Tendencias, Max/Min, Avg, ratios
    TendenciaYmuchomas( dataset, 
                        cols= cols_tendencias,
                        ventana=  ventana,
                        tendencia= TRUE,
                        minimo=    TRUE,
                        maximo=    TRUE,
...
# Calculo lags para todas las columnas
if (length(klags) > 0) {
  cols_lagueables  <- copy( setdiff( colnames(dataset), kcampos_fijos ) )
  
  for (lag in klags) {
    Lags( cols_lagueables, lag, kdelta_lags )   #calculo los lags de orden "lag"
  }
...
# Proceso combinaciones de variables más importantes si el feature está activado
importancia = ProcesarCombinaciones(importancia)

# Trunco las primeras X variables importantes
if (ktruncar_n_vars > 0) {
  cols_finales = c(importancia[pos <= ktruncar_n_vars, Feature], kcampos_fijos)
