...
AgregarVariables  <- function( dataset )
{
  ...
  ReportarCampos( dataset )
  cat( "Fin agregado variables manuales\n")
}
...
{
  gc()
  cat("Aplicando lags de orden: ", nlag, "\n")
  ReportarCampos(dataset)
...
{
  gc()
  cat("Aplicando tendencias y mucho mas con ventana: ", ventana, "\n")
  ReportarCampos(dataset)
  ...
  ReportarCampos(dataset)
  cat("Fin de tendencias y mucho mas con ventana: ", ventana, "\n")
}
...
#
# Devuelve un vector con los nombres de las nuevas columnas creadas
#------------------------------------------------------------------------------

CruzarVariablesImportantes <- function(dataset, tb_importancia, funcion_cruza, n=100)
{
  gc()
  ...
  if (PARAM$cruzas > 0) {
    ...
    CruzarVariablesImportantes(
      ...
      function(dataset, c1, c2) {
        ...
        dataset[, paste0("cruza_avg_", c1, '_vs_', c2) := rowSums( cbind( get(c1),  get(c2) ) , na.rm=T ) / 2]
...
ProcesoCompleto <- function(n_vuelta) {
  ...
  cols_a_procesar  <- copy( setdiff( colnames(dataset), PARAM$const$campos_fijos ) )
  ProcesarTendencias(cols_a_procesar)
  
  cols_a_procesar  <- copy( setdiff( colnames(dataset), PARAM$const$campos_fijos ) )
  ProcesarLags(cols_a_procesar)
