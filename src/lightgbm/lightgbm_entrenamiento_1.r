...
kbayesiana     <- "lightGBM.RDATA"
klog           <- "lightGBM_log.txt"
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  early_stop = as.integer(50 + 5/x$learning_rate)
  early_stop = ifelse(early_stop > 200, 200, early_stop)
  
  #el parametro discolo, que depende de otro
  param_variable  <- list(  early_stopping_rounds= early_stop )
